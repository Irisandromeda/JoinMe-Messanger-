...
extension UIButton {
    ...
    func buttonWithFacebookLogo() {
        let facebookLogo = UIImageView(image: #imageLiteral(resourceName: "facebook"), contentMode: .scaleAspectFit)
        facebookLogo.translatesAutoresizingMaskIntoConstraints = false
        self.addSubview(facebookLogo)
        facebookLogo.leadingAnchor.constraint(equalTo: self.leadingAnchor, constant: 30).isActive = true
        facebookLogo.centerYAnchor.constraint(equalTo: self.centerYAnchor).isActive = true
    }
